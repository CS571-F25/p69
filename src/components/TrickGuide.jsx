...
export default function TrickGuide() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <h1 className="text-4xl font-bold mb-2">Trick Guide</h1>
      <p className="text-gray-400 mb-2 text-sm">IWWF Waterski Rules 2025</p>
      <a
        href={trickRulesPdf}
        target="_blank"
        rel="noopener noreferrer"
        className="inline-block mb-6 bg-blue-900 hover:bg-blue-700 text-gray-100 font-light text-sm tracking-wide px-4 py-2 rounded-lg transition-all duration-200 border border-blue-800 hover:border-blue-600"
      >
        📄 View Full IWWF 2025 Trick Rules PDF
      </a>

      {/* Surface Turns Section */}
      <div className="mb-8">
        <h2 className="text-2xl font-light mb-4 text-blue-400 border-b border-slate-700 pb-2">
          SURFACE TURNS
        </h2>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-slate-800 border-b border-slate-700">
                <th className="text-left p-3 text-blue-400 font-medium">CODE</th>
                <th className="text-left p-3 text-blue-400 font-medium">DESCRIPTION</th>
                <th className="text-center p-3 text-blue-400 font-medium">NO.</th>
                <th className="text-center p-3 text-blue-400 font-medium border-l border-slate-700" colSpan="2">2 SKIS</th>
                <th className="text-center p-3 text-blue-400 font-medium border-l border-slate-700" colSpan="2">1 SKI</th>
              </tr>
              <tr className="bg-slate-800/50 border-b border-slate-700 text-xs">
                <th></th>
                <th></th>
                <th></th>
                <th className="text-center p-2 text-gray-400 border-l border-slate-700">BAS</th>
                <th className="text-center p-2 text-gray-400">REV</th>
                <th className="text-center p-2 text-gray-400 border-l border-slate-700">BAS</th>
                <th className="text-center p-2 text-gray-400">REV</th>
              </tr>
            </thead>
            <tbody>
              {surfaceTurns.map((trick, index) => (
                <tr key={index} className="border-b border-slate-700/50 hover:bg-slate-800/30">
                  <td className="p-3 font-mono text-blue-300 font-medium">{trick.code}</td>
                  <td className="p-3 text-gray-300">{trick.description}</td>
                  <td className="p-3 text-center text-gray-400">{trick.no}</td>
                  <td className="p-3 text-center border-l border-slate-700/50 text-gray-200">{trick.ski2Bas}</td>
                  <td className="p-3 text-center text-gray-200">{trick.ski2Rev}</td>
                  <td className="p-3 text-center border-l border-slate-700/50 text-gray-200">{trick.ski1Bas}</td>
                  <td className="p-3 text-center text-gray-200">{trick.ski1Rev}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 space-y-3">
        ...
        <div className="border-t border-slate-700 pt-3">
          <p className="text-gray-400 text-sm font-medium mb-2">When calling trick runs, the following substitutions will be allowed:</p>
