export default function TrickButton({ abbr, points, onClick, disabled = false, alreadyPerformed = false }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`font-light text-base sm:text-lg tracking-wide px-3 py-3 sm:px-6 sm:py-4 rounded-lg transition-all duration-200 border relative ${
        disabled
    ...
    >
      ...
      <div className={`text-xs sm:text-sm ${disabled ? "text-gray-600" : "text-gray-400"}`}>
